...
import MapCampusPoints from '../components/MapCampusPoints'
...
export default function DistrictDetail() {
  ...
  useEffect(() => {
    let alive = true
    ;(async () => {
      try { const d = await api.getDistrict(id); if (alive) setDetails(d) } catch {}
  ...
  const center = [31, -99] // TODO: compute from district geometry later
  ...
  return (
    <div className="container" style={{paddingTop:12}}>
      ...
      {loading ? <KpiRow /> :
        <div className="grid">
          <div className="card kpi"><div className="k-label">Enrollment</div><div className="k-val">{fmtMaybe(details?.enrollment)}</div></div>
      ...
      {loading ? <MapBox /> : <MapCampusPoints districtId={id} center={center} zoom={9} />}
    </div>
  )
}
