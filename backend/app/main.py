...
DATA_DIR = os.getenv("DATA_DIR", "../data/raw")          # your CSV/GeoJSON folder
CORS = os.getenv("CORS_ORIGINS", "*")                    # e.g. "https://lone-star-ledger4.vercel.app"
...
def _read_geo(path: str) -> Dict[str, Any]:
    if not os.path.exists(path):
...
def _cacheable_json(data: Any, max_age: int = 3600) -> Response:
...
_cache: Dict[str, Any] = {
    "districts": None,       # list of dicts
    "schools": None,         # list of dicts
    "geo_districts": None,   # GeoJSON
    "geo_campuses": None,    # GeoJSON
    "by_district": {},
    "by_school": {},
    "loaded_at": None,
}
...
def _load_all():
    ...
    debts   = _read_csv(debt_path)
    campus  = _read_csv(campus_path)

    # create a quick map of debt by district id
    debt_map: Dict[str, Dict[str, Any]] = {
    ...
    for r in finances:
        ...
        if total == 0:
            # fallback: sum common category fields if they exist
            cat_keys = [
                "instruction","instructional","support services","general administration",
    ...
    # GeoJSONs: prefer processed if present
...
_request_counts = {"summary":0, "districts":0, "schools":0, "geo_d":0, "geo_c":0}
...
def districts(
    ...
    _ensure_loaded()
    _request_counts["districts"] += 1
    items = list(_cache["districts"])
    if q:
...
def schools(
    ...
    _ensure_loaded()
    _request_counts["schools"] += 1
    items = list(_cache["schools"])
    if q:
...
# GeoJSON (cached with Cache-Control)
@app.get("/geojson/districts")
def geojson_districts():
    _ensure_loaded()
    _request_counts["geo_d"] += 1
    return _cacheable_json(_cache["geo_districts"], max_age=3600)

@app.get("/geojson/campuses")
def geojson_campuses():
    _ensure_loaded()
    _request_counts["geo_c"] += 1
    return _cacheable_json(_cache["geo_campuses"], max_age=3600)

# ------------------------------------------------------------------------------
# Newsletter capture (prototype: store on ephemeral disk)
# ------------------------------------------------------------------------------
...
def newsletter_signup(payload: Dict[str, Any] = Body(...), request: Request = None):
    ...
    out_path = "/tmp/newsletter.csv"  # Render ephemeral disk; fine for prototype
...
# ------------------------------------------------------------------------------
# CSV export for filtered results (districts & schools)
# ------------------------------------------------------------------------------

@app.get("/export/districts", response_class=PlainTextResponse)
def export_districts(
    q: Optional[str] = None,
    min_per_pupil: Optional[float] = None,
    max_per_pupil: Optional[float] = None
):
    data = districts(q=q, min_per_pupil=min_per_pupil, max_per_pupil=max_per_pupil,
                     limit=10_000, offset=0)["items"]
    out = io.StringIO()
    w = csv.DictWriter(out, fieldnames=["district_6","district_name","enrollment","total_spend","per_pupil_spend","debt_total"])
    w.writeheader()
    for r in data:
        w.writerow(r)
    return out.getvalue()

@app.get("/export/schools", response_class=PlainTextResponse)
def export_schools(
    q: Optional[str] = None,
    min_reading: Optional[float] = None,
    min_math: Optional[float] = None
):
    data = schools(q=q, min_reading=min_reading, min_math=min_math,
                   limit=10_000, offset=0)["items"]
    out = io.StringIO()
    w = csv.DictWriter(out, fieldnames=["campus_9","campus_name","district_6","reading_on_grade","math_on_grade"])
    w.writeheader()
    for r in data:
        w.writerow(r)
    return out.getvalue()
...
# ------------------------------------------------------------------------------
# Prometheus-style counters (very simple)
# ------------------------------------------------------------------------------

@app.get("/metrics", response_class=PlainTextResponse)
def metrics():
    lines = [
        "# HELP lonestar_requests_total Request counts by endpoint",
        "# TYPE lonestar_requests_total counter",
    ]
    for k, v in _request_counts.items():
        lines.append(f'lonestar_requests_total{{endpoint="{k}"}} {int(v)}')
    return "\n".join(lines) + "\n"
